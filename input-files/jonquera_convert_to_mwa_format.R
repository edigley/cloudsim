...
ds <- read.table("/home/edigley/eclipse-workspace/cloudsim/input-files/jonquera_single_core_8_workers_trace.txt", header=T)
...
ds <- subset(merge(ds,nJobs),!is.na(time), select=c("taskId","time","jobId", "jobSize", "runtime", "user", "peer", "TraceID", "Cluster.IAT", "Cluster.RT"))
ds <- ds[order(ds$taskId),]

write.table(ds, "/home/edigley/eclipse-workspace/cloudsim/input-files/jonquera_single_core_8_workers_trace_mwa.txt",col.names=T,row.name=F,quote=F)
